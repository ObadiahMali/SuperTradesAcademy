...
@php
  ...
  // Original plan price from config
  ...
  // Totals in UGX
  $totalPaidUGX = $payments->sum(fn($p) => $p->amount_converted ?? 0);
  $balanceUGX   = max(($student->course_fee ?? 0) - $totalPaidUGX, 0);
...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <div>
    <a href="{{ route('secretary.payments.create', $student) }}" class="btn btn-success me-2">Add Payment</a>
    <a href="{{ route('secretary.payments.receipt', $student) }}" target="_blank" class="btn btn-outline-primary">
      Print Summary
    </a>
  </div>
</div>
  ...
  <div class="col-lg-4">
    <div class="card p-3">
      ...
      <div class="mt-2">
        ...
        <hr>
        <div><strong>Total Price (UGX):</strong> UGX {{ number_format($student->course_fee ?? 0, 2) }}</div>
        <div><strong>Total Paid:</strong> UGX {{ number_format($totalPaidUGX, 2) }}</div>
        <div><strong>Balance:</strong>
          @if($balanceUGX <= 0)
            <span class="text-success">Paid in full</span>
          @else
            <span class="text-danger">UGX {{ number_format($balanceUGX, 2) }}</span>
          @endif
        </div>
      </div>
    </div>
    ...
    <div class="card p-3">
        ...
        <tbody>
          @forelse($payments as $p)
            <tr>
              ...
              <td>UGX {{ number_format($p->amount_converted ?? 0, 0) }}</td>
              <td class="muted-small">{{ $p->method ?? '—' }}</td>
              <td class="muted-small">{{ $p->reference ?? $p->receipt_number ?? '—' }}</td>
              <td class="text-end">
                <a href="{{ route('secretary.students.receipt', $p) }}" target="_blank" class="btn btn-sm btn-outline-secondary">Print</a>
              </td>
            </tr>
          @empty
            <tr>
      ...
      <div class="d-flex justify-content-end mt-3">
        <div class="me-4 text-end">
          ...
          <div class="fw-bold">UGX {{ number_format($student->course_fee ?? 0, 2) }}</div>
        </div>

        <div class="me-4 text-end">
          <div class="muted-small">Paid</div>
          <div class="fw-bold">UGX {{ number_format($totalPaidUGX, 2) }}</div>
        </div>
