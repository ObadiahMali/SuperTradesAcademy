...
/* High specificity fallback if other rules override */
.card.section-card .section-title,
.card.section-card .section-sub,
.card.section-card .section-body { /* no-op but increases specificity */ }
...
/* Outline variant if you prefer white background with red border */
.btn-danger-outline {
  background: #ffffff;
  color: #b91c1c;
  border: 1px solid #fca5a5;
}
  /* Ensure action areas are visible and buttons stand out */
.form-actions {
  display: flex;
  gap: 10px;
  justify-content: flex-end;
...
/* Danger button (delete) — solid red background and white text */
.btn-danger {
  background: #dc2626; /* red-600 */
  color: #fff;
    ...
    <aside class="card profile-summary">
      <div class="profile-head">
        ...
        <div>
          ...
          <p class="meta">Member since {{ optional(auth()->user()->created_at)->format('M Y') ?? '—' }}</p>
        </div>
      </div>

      <div class="info-block">
        <p class="muted"><strong>User ID</strong>: {{ auth()->id() ?? '—' }}</p>
        <p class="muted" style="margin-top:6px;"><strong>Role</strong>: {{ implode(', ', auth()->user()?->getRoleNames()->toArray() ?? ['—']) }}</p>
      ...
      <div class="debug-badge">DEBUG: profile page loaded — user id {{ auth()->id() }}</div>
    </aside>
